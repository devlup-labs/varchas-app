...
class _MyCompetitionScreenState extends State<MyCompetitionScreen> {
  ...
  void initState() {
    ...
    updateTeamData();
    getTeamId().then((value) {
      teamId = value;
      getScheduleResults(currentDay, teamId: teamId).then((results) {
  ...
  Future<String> getTeamId() async{
    ...
    var td = prefs.getStringList('teamData');
    return td![0];
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            Header(size: data, screenName: "My Competitions"),
